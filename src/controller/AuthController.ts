...
export default {
  async authenticate(request: CustomRequest, response: Response) {
    ...
    if(!user) return response.status(401).json({ error_message: "User dose not exists"})
   
    const passwordIsValid = await compare(password, user.password)

    if(!passwordIsValid) return response.status(401).json({ error_message: "User not Authorized, password is incorrect" })
